...
class ArtistService(PublicArtService):
    ...
    def get_items_by_page(self, start_item, page_length):
        ...
        artists = Artist.query.paginate(page, page_length, False).items
    ...
    def get_filtered_items_by_page(self, search_value, start_item, page_length):
        page = (start_item / page_length) + 1
        artists = (
            Artist.query.filter(ArtistService._filter_by_search(search_value))
    ...
    def _form_results(data):
        return [
            {"name": f"{item.first_name} {item.last_name}", "info": item.biography}
            for item in data
        ]

    @staticmethod
    def _filter_by_search(search_value):
        return Artist.first_name.ilike(f"%{search_value}%") | Artist.last_name.ilike(
            f"%{search_value}%"
        )
...
class ArtworkService(PublicArtService):
    ...
    def get_items_by_page(self, start_item, page_length):
        page = (start_item / page_length) + 1
        artwork = Artwork.query.paginate(page, page_length, False).items
    ...
    def get_filtered_items_by_page(self, search_value, start_item, page_length):
        page = (start_item / page_length) + 1
        artwork = (
            Artwork.query.filter(ArtworkService._filter_by_search(search_value))
            .paginate(page, page_length, False)
            .items
        )
        return ArtworkService._form_results(artwork)
...
class NeighbourhoodArtworkService(ArtworkService):
    ...
    def get_items_by_page(self, start_item, page_length):
        page = (start_item / page_length) + 1
        artwork = (
            Artwork.query.filter(Artwork.neighbourhood == self.neighbourhood)
            .paginate(page, page_length, False)
            .items
        )
        return ArtworkService._form_results(artwork)
    ...
    def get_filtered_items_by_page(self, search_value, start_item, page_length):
        ...
        artwork = (
            Artwork.query.filter(Artwork.neighbourhood == self.neighbourhood)
            .filter(ArtworkService._filter_by_search(search_value))
            .paginate(page, page_length, False)
            .items
        )
        return ArtworkService._form_results(artwork)
